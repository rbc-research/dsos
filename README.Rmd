...
lags behind. I recommend the package on GitHub over CRAN for now. The package
on CRAN (v0.1.0) will eventually be updated to reflect recent changes. From
CRAN, install the package with:
...
Use simulated outlier scores to test for no adverse shift when the null (no
...
Here, the outlier scores are from isolation forest for
...
`dsos` also implements a method for confidence-based OOD detection
via prediction uncertainty. Users can easily bring their own scores via the
